...
def worknet_list(is_yes):
    ...
    while True:
        ...
        file_name = f'../list/{pageIndex}.page'
    ...
    pd.DataFrame(result).to_csv('../list/worknet.csv', index=False)    
    logging.info(f"worknet crawl list end:{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")


if __name__=='__main__':
    parser = argparse.ArgumentParser(prog='jobkorea list crawle', description='worknet 리스트를 크롤합니다') 
